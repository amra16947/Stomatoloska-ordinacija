...
$upit = $veza->prepare("SELECT ime FROM korisnik WHERE username=?;");
...
$korisnici = $upit->fetchAll();
	 ...
	 /*if(file_exists('Baza/korisnici.xml'))
	  ...
	  if(!$korisnici) /*ako ne postoji username*/
	  {
	  $uloga="gost";
	  $upit = $veza->prepare("INSERT INTO korisnik ( uloga, ime, prezime, username, sifra) VALUES (?, ?, ?, ?, ?);");
			$upit->bindValue(1, $uloga, PDO::PARAM_STR);
			$upit->bindValue(2, $ime, PDO::PARAM_STR);
			$upit->bindValue(3, $prezime, PDO::PARAM_STR);
			$upit->bindValue(4, $username, PDO::PARAM_STR);
			$upit->bindValue(5, $sifra, PDO::PARAM_STR);
		    ...
		    else
		    {
				...
				$poruka = "Uspješno dodan korisnik";
		    }
	  }
	  else
	  {
